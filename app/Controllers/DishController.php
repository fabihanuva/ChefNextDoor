...
class DishController extends Controller {
    ...
    public function store() {
        try {
            ...
            // Validation
            if (!$title || $price <= 0) {
                Session::set('error', 'Valid title and positive price are required.');
                header("Location: " . url('/dishes/create'));
                exit;
            }

            if ($stock < 0) {
                Session::set('error', 'Stock cannot be negative.');
                header("Location: " . url('/dishes/create'));
                exit;
            }
            ...
            Session::set('error', 'An error occurred while saving the dish.');
            header("Location: " . url('/dishes/create'));
            exit;
        }
    }
    ...
    public function update() {
        try {
            ...
            error_log($e->getMessage());
            Session::set('error', 'An error occurred while updating the dish.');
            header("Location: " . url('/dishes/edit?id=' . $id));
            exit;
        }
    }

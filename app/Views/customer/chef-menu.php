...
<script>
    ...
    async function fetchDishes(query) {
        ...
        try {
            ...
            data.forEach(d => {
                const imageHtml = d.image ? `<img src="/ChefNextDoor/uploads/dishes/${d.image}" class="w-full h-40 sm:h-44 object-cover" />` : '';
                    ...
                    <div class="bg-white rounded-2xl border border-brand-100 overflow-hidden">
                        ...
                        <div class="p-4">
                            <div class="flex justify-between mb-1">
                                ...
                                <span class="text-brand-600 font-bold">৳${new Intl.NumberFormat().format(d.price)}</span>
                            </div>
                            <p class="text-xs text-gray-400 mb-3">${d.description || ''}</p>
                            <div class="flex justify-between items-center">
                                <span class="text-xs bg-brand-50 px-2 py-1 rounded">${d.category || 'Other'}</span>
                                <div class="flex items-center gap-2">
                                    <form method="POST" action="<?= url('/cart/add') ?>">
                                        <?= csrf_field() ?>
                                        <input type="hidden" name="dish_id" value="${d.id}">
